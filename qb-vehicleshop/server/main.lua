...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    if (balance - vData["price"]) >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "نجاح! تم تسليم سيارتك إلى "..QB.GarageLabel[garage], "success", 5000)
        ...
        TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "السيارة المشتراة", "green", "**"..GetPlayerName(src) .. "** لديه " .. vData["name"] .. " اشتريت من اجل €" .. vData["price"])
    else
...
AddEventHandler('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle, class)
    ...
    if (balance - vehiclePrice) >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Success! Your vehicle is waiting for you outside.", "success", 5000)
        ...
        TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "Vehicle purchased (showroom)", "green", "**"..GetPlayerName(src) .. "** has a " .. QBCore.Shared.Vehicles[vehicle]["name"] .. " bought for DZA" .. QBCore.Shared.Vehicles[vehicle]["price"])
    else
        TriggerClientEvent("QBCore:Notify", src, "You don't have enough money, you miss DZA"..format_thousand(vehiclePrice - balance), "error", 5000)
    end
end)
...
QBCore.Commands.Add("sellcar", "بيع المركبة", {{name="Id", help="ID OF PLAYER"},{name="price", help="Price Of Car"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "cardealer" then
        if TargetId ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', source, 'يجب وضع معرف اللاعب', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'أنت لست عامل في المعرض', 'error')
    end
end)

QBCore.Commands.Add("testdrive", "أخذ قيادة تجريبة", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local TargetId = args[1]
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicle', function(ShowroomVehicle)
    ...
    elseif Player.PlayerData.money.bank >= VehPrice then
        ...
        if Player.PlayerData.money.cash > Player.PlayerData.money.bank then
            TriggerClientEvent('QBCore:Notify', src, 'ليس لديك ما يكفي من المال .. تفتقد ('..(Player.PlayerData.money.cash - VehPrice)..',-)')
        else
            TriggerClientEvent('QBCore:Notify', src, 'ليس لديك ما يكفي من المال .. تفتقد ('..(Player.PlayerData.money.bank - VehPrice)..',-)')
        end
    end
end)
