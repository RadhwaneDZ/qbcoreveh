...
function GetCatVehicles(catergory)
    ...
    for k, v in pairs(shopVehicles[catergory]) do
        Menu.addButton(shopVehicles[catergory][k].name, "SelectVehicle", v, catergory, "DZA"..shopVehicles[catergory][k]["price"])
    end
end
                ...
                if VehicleData["shop"] == "pdm" then
                    ...
                    if SellDistance < 1 then
                        if not SellStarted then
                            DrawText3Ds(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z, '~b~E~w~ - ﻞﺑﺎﻘﻣ ﺔﺒﻛﺮﻤﻟﺍ ﻊﻴﺑ ~g~$'..math.ceil(VehicleData["price"] / 100 * 60))
                            if IsControlJustPressed(0, Keys["E"]) then
                                SellStarted = true
                            end
                        else
                            DrawText3Ds(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z, '~b~7~w~ - ﺪﻴﻛﺄﺗ / ~b~8~w~ - ﺀﺎﻐﻟﺇ')
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                ...
                                QBCore.Functions.TriggerCallback('qb-vehicleshop:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        TriggerEvent("qb-phone-new:client:BankNotify", "لقد حصلت على"..math.ceil(VehicleData["price"] / 100 * 60).." لبيع المركبة")
                                        local veh = GetVehiclePedIsIn(ped)
                                        QBCore.Functions.DeleteVehicle(veh)
                                    else
                                        QBCore.Functions.Notify('هذه ليست مركبتك..', 'error')
                                    end
                ...
                if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                    ...
                    if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                        if not vehshop.opened then
                            if not buySure then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.8, '~g~G~w~ - ﺔﺒﻛﺮﻤﻟﺍ ﺮﻴﻐﺗ  (~g~'..displayName..'~w~)')
                            end
                            if not buySure then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.70, '~g~E~w~ - ﺔﺒﻛﺮﻤﻟﺍ ﺀﺍﺮﺷ (~g~€'..vehPrice..'~w~)')
                            elseif buySure then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'ﺪﻛﺎﺘﻣ ﺖﻧﺃ ﻞﻫ? | ~g~[7]~w~ ﻢﻌﻧ -/- ~r~[8]~w~ ﻻ')
                            end
                        elseif vehshop.opened then
                            if modelLoaded then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'ﺔﺒﻛﺮﻤﻟﺍ ﺀﺎﻘﺘﻧﺇ')
                            else
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'ﺔﺒﻛﺮﻤﻟﺍ ﻞﻘﻧ ﻢﺘﻳ..')
                            end
                        end
                    else
                        DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'ﺔﺒﻛﺮﻤﻟﺍ ﻡﺍﺪﺨﺘﺳﺇ ﻢﺘﻳ...')
                    end
                    ...
                    if IsDisabledControlJustPressed(0, Keys["8"]) then
                        QBCore.Functions.Notify('لم تقم بشراء المركبة', 'error', 3500)
                        buySure = false
                    end
                    DisableControlAction(0, Keys["7"], true)
                    DisableControlAction(0, Keys["8"], true)
                elseif QB.ShowroomVehicles[ClosestVehicle].inUse then
                    DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 0.5, 'ﺔﺒﻛﺮﻤﻟﺍ ﻡﺍﺪﺨﺘﺳﺇ ﻢﺘﻳ')
                end
