...
Citizen.CreateThread(function()
    for k, v in pairs(QB.VehicleShops) do
        ...
        AddTextComponentSubstringPlayerName("<FONT FACE='Arb'> ﺔﻤﺨﻔﻟﺍ ﺽﺭﺎﻌﻤﻟﺍ </font>")
            ...
            if dist < 2 then
                if PlayerJob ~= nil then
                    if PlayerJob.name == "cardealer" then
                        DrawText3Ds(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.9, '~g~G~w~ - ﺔﺒﻛﺮﻤﻟﺍ ﺮﻴﻐﺗ')
                        DrawText3Ds(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.75, '~b~/sellcar [id]~w~ - ﺔﺒﻛﺮﻤﻟﺍ ﻊﻴﺑ ~b~/testdrive~w~ - ﺓﺩﺎﻴﻗ ﺭﺎﺒﺘﺧﺇ')
                            ...
                            if QBCustom.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if QBCustom.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if QBCore.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        DrawText3Ds(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, '~g~7~w~ - ﺀﺍﺮﺸﻠﻟ / ~r~8~w~ - ﺀﺎﻐﻟﺇ - ~g~($'..QBCore.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                                        ...
                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            QBCore.Functions.Notify('لم تقم بشراء المركبة')
                                            QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
AddEventHandler('qb-vehicleshop:client:SellCustomVehicle', function(TargetId)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            ...
            QBCore.Functions.Notify("أنت لست بالقرب من المركبة", "error")
        end
    else
        QBCore.Functions.Notify("لا أحد قريب", "error")
    end
end)
