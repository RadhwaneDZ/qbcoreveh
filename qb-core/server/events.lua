...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	if name == nil then 
		QBCore.Functions.Kick(src, 'الرجاء عدم استخدام اسم ستيم فارغ', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	if(string.match(name, "[*%%'=`\"]")) then
        QBCore.Functions.Kick(src, 'الرجاء عدم استخدام رموز ('..string.match(name, "[*%%'=`\"]")..') غير مسموح ، الرجاء تغييره', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	if (string.match(name, "drop") or string.match(name, "table") or string.match(name, "database")) then
        QBCore.Functions.Kick(src, 'اسمك يحتوي على (drop/table/database) وهذا ممنوع.\nالرجاء تغيير', setKickReason, deferrals)
        CancelEvent()
        return false
	end
    ...
    if (QBConfig.IdentifierType == "steam" and (steamid:sub(1,6) == "steam:") == false) then
        QBCore.Functions.Kick(src, 'الرجاء تشغيل ستيم.', setKickReason, deferrals)
        CancelEvent()
		return false
...
AddEventHandler('QBCore:UpdatePlayer', function(data)
	...
	if Player ~= nil then
		...
		TriggerClientEvent('QBCore:Notify', src, "لقد حصلت على راتبك :  DZA"..Player.PlayerData.job.payment)
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "انت خارج الخدمة!")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "تم تسجيل دخولك بالخدمة!")
	end
